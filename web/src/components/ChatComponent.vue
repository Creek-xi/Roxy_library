<template>
  <div class="chat" ref="chatContainer">
      ...
      <div class="header__left">
        ...
        <a-dropdown>
          <a class="ant-dropdown-link" @click.prevent>
            <UserOutlined style="margin-right: 3px;" />
            <span class="text">{{ meta.selectedRole || '选择角色' }}</span>
          </a>
          <template #overlay>
            <a-menu>
              <a-menu-item v-for="(role, index) in roles" :key="index" @click="selectRole(role)">
    ...
    <div v-if="conv.messages.length == 0" class="chat-examples">
      <h1>你好，我是{{ meta.selectedRole || 'Roxy' }}，有什么想问的吗？</h1>
      <div class="opts">
        <div
          class="opt__button"
          ...
          <div class="options__left">
            ...
            >
              <a class="ant-dropdown-link" @click.prevent>
                ...
                <span class="text">{{ meta.selectedKB === null ? '不使用图书馆' : opts.databases[meta.selectedKB]?.name }}</span>
              </a>
              <template #overlay>
                <a-menu>
...
const meta = reactive(JSON.parse(localStorage.getItem('meta')) || {
  ...
  history_round: 5,
  db_name: null,
  selectedRole: null, // 添加 selectedRole 属性
  systemPrompt: '', // 添加 systemPrompt 属性
})

const roles = ref(['Roxy', 'Tomoyo']) // 定义角色列表
